...
import (
	clientc "clients/gen/http/client/client"
	"flag"
	"fmt"
	"net/http"
	"os"
...
Usage:
    %s [globalflags] client COMMAND [flags]
...
Additional help:
    %s client COMMAND --help
`, os.Args[0], os.Args[0])
}
func clientAddUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] client add -body JSON -client-id STRING
...
func clientGetUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] client get -client-id STRING

Get implements get.
    -client-id STRING: Client ID

Example:
    `+os.Args[0]+` client get --client-id "Tempore recusandae qui dolor veritatis."
`, os.Args[0])
}

func clientShowUsage() {
	fmt.Fprintf(os.Stderr, `%s [flags] client show

Show implements show.

Example:
    `+os.Args[0]+` client show
`, os.Args[0])
}
